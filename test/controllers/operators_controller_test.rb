...
class OperatorsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create operator" do
    assert_difference('Operator.count') do
      post operators_url, params: { operator: { email: @operator.email, name: @operator.name, phone: @operator.phone } }
    end
  ...
  test "should update operator" do
    patch operator_url(@operator), params: { operator: { email: @operator.email, name: @operator.name, phone: @operator.phone } }
